...
class SteganographyUtils:
    ...
    def _ssim(self, img1, img2, window, window_size, channel, size_average=True):
        ...
        sigma1_sq = (
            conv2d(img1 * img1, window, padding=padding_size, groups=channel) - mu1_sq
        )
        sigma2_sq = (
            conv2d(img2 * img2, window, padding=padding_size, groups=channel) - mu2_sq
        )
        sigma12 = (
            conv2d(img1 * img2, window, padding=padding_size, groups=channel) - mu1_mu2
        )
...
class DenseEncoder(nn.Module):
    def __init__(self, data_depth=8):
        ...
        self.conv4 = nn.Sequential(nn.Conv2d(96, 3, kernel_size=3, stride=1, padding=1))
...
class Evaluate(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Sequential(
            nn.Linear(2, 64), nn.ReLU(inplace=True), nn.Linear(64, 1)
        )

    def forward(self, encoded_image, decoded_data):
        # Process the encoded image with image_layers
        encoded_image_score = self.pool(self.image_layers(encoded_image)).view(
            encoded_image.size(0), -1
        )

        # Process the decoded data with data_layers
        decoded_data_score = self.pool(self.data_layers(decoded_data)).view(
            decoded_data.size(0), -1
        )
    ...
    def forward(self, image, data):
        ...
        for i in range(0, H, self.block_size):
            for j in range(0, W, self.block_size):
                ...
                decoded_data[:, :, i : i + self.block_size, j : j + self.block_size] = (
                    decoded_block
                )
...
class CoverImageDataset(Dataset):
    def __init__(self, json_path, image_base_dir, block_size=64, transform=None):
        ...
        # Extract image information from the JSON structure
        self.images = []
        for img_data in data["images"]:
            if isinstance(img_data, dict):
                self.images.append(img_data["file_name"])
            else:
                # Handle the case where img_data might be just the filename
    ...
    def __getitem__(self, idx):
        img_name = os.path.join(self.image_base_dir, self.images[idx])
        try:
...
json_path = "/home/dxv2k/.cache/kagglehub/datasets/dntai2/cocostuff-10k-v1-1/versions/1/cocostuff-10k-v1.1.json"
image_base_dir = "/home/dxv2k/.cache/kagglehub/datasets/dntai2/cocostuff-10k-v1-1/versions/1/cocostuff-10k-v1.1/images/"  # Base directory containing images
...
# Create dataset instance with new parameters
cover_image_dataset = CoverImageDataset(
    json_path=json_path,
...
if __name__ == "__main__":
    ...
    optimizer_enc_dec = optim.AdamW(
        list(model.encoder.parameters()) + list(model.decoder.parameters()), lr=0.0001
    )
    ...
    for epoch in range(num_epochs):
        ...
        for images in progress_bar:
            ...
            progress_bar.set_postfix(
                critic_loss=loss_c.item(), enc_dec_loss=total_loss.item()
            )
